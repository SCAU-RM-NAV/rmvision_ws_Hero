...
bool BaSolver::solveBa(const std::deque<Armor> & armors, cv::Mat & rmat) noexcept
{
  ...
  for (const auto & armor : armors) {
    ...
    double initial_armor_pitch = armor.number == "7" ? -FIFTTEN_DEGREE_RAD : FIFTTEN_DEGREE_RAD;
    ...
    EdgeProjection * edge = new EdgeProjection(
      Sophus::SO3d(camera2imu), armor_position_3d, cam_internal_k_, initial_armor_size,
      initial_armor_pitch);
    edge->setId(id_counter + optimized_frame_number);
    edge->setVertex(0, v_yaw);
    edge->setMeasurement(armor_landmarks_2d);
    edge->setInformation(EdgeProjection::InfoMatrixType::Identity());  //g2o库自带函数
  ...
  double pitch_optimized = armors.back().number == "7" ? -FIFTTEN_DEGREE_RAD : FIFTTEN_DEGREE_RAD;
