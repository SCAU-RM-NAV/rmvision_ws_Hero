...
void ArmorDetectorNode::imageCallback(const sensor_msgs::msg::Image::ConstSharedPtr img_msg)
{
  ...
  // RCLCPP_DEBUG_STREAM(this->get_logger(), "Received image, latency: " << latency << "ms");

  // auto start_time = this->now();
  auto armors = detectArmors(img_msg);
  // auto end_time = this->now();
  // latency = (end_time - start_time).seconds() * 1000;
  // RCLCPP_DEBUG_STREAM(this->get_logger(), "Detect image, latency: " << latency << "ms");
  ...
  if (pnp_solver_ != nullptr) {
    ...
    for (auto & armor : armors) {
      ...
      if (success) {
        ...
        armor_msg.armor_ratio = detector_->armor_radio_;
        // Fill the markers
        armor_marker_.id++;
    ...
    all_ba_ =true;
    // std::cout<<"ba time:"<<ba_time<<std::endl;
    // Publishing marker
    publishMarkers();
  }
}
...
std::unique_ptr<Detector> ArmorDetectorNode::initDetector()
{
  ...
  Detector::ArmorParams a_params = {
    ...
    .max_large_armor_ratio = declare_parameter("armor.max_large_armor_ratio", 15.5),
  ...
  std::vector<std::string> ignore_classes =
    this->declare_parameter("ignore_classes", std::vector<std::string>{"0"});
  bool capture = this->declare_parameter("capture", false);
  detector->classifier =
    std::make_unique<NumberClassifier>(model_path, label_path, threshold, capture, ignore_classes);
  ...
  const sensor_msgs::msg::Image::ConstSharedPtr & img_msg)
{
  ...
  if (detector_->detect_color == RED)
    detector_->binary_thres = get_parameter("binary_thres_red").as_int();
  else
    detector_->binary_thres = get_parameter("binary_thres_blue").as_int();
...
void ArmorDetectorNode::updateDetectorParams()
{
  ...
  Detector::ArmorParams a_params = {
    .min_light_ratio = get_parameter("armor.min_light_ratio").as_double(),
