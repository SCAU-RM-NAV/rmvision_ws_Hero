...
{
class HikCameraNodeB : public rclcpp::Node
{
public:
  explicit HikCameraNodeB(const rclcpp::NodeOptions & options) : Node("hik_camera_b", options)
  {
    RCLCPP_INFO(this->get_logger(), "Starting HikCameraNode_B!");
    ...
    auto camera_info_url_B =
      this->declare_parameter("camera_info_url_B", "package://hik_camera/config/camera_info_B.yaml");
    if (camera_info_manager_->validateURL(camera_info_url_B)) {
      camera_info_manager_->loadCameraInfo(camera_info_url_B);
      camera_info_msg_ = camera_info_manager_->getCameraInfo();
    } else {
      RCLCPP_WARN(this->get_logger(), "Invalid camera info URL: %s", camera_info_url_B.c_str());
    }

    params_callback_handle_ = this->add_on_set_parameters_callback(
      std::bind(&HikCameraNodeB::parametersCallback, this, std::placeholders::_1));

    cam_gimbal_fdb_sub_ = this->create_subscription<auto_aim_interfaces::msg::GimbalFdb>(
    "/gimbal_fdb", rclcpp::SensorDataQoS(),
    std::bind(&HikCameraNodeB::camGimbalFdbCallback, this, std::placeholders::_1));
...
// }
  ...
  image_transport::CameraPublisher camera_pub_B;
...
RCLCPP_COMPONENTS_REGISTER_NODE(hik_camera::HikCameraNodeB)
