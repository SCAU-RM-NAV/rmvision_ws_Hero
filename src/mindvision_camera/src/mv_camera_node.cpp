...
public:
  explicit MVCameraNode(const rclcpp::NodeOptions & options) : Node("mv_camera", options)
  {
    ...
    std::bind(&MVCameraNode::camGimbalFdbCallback, this, std::placeholders::_1));
...
private:
  void declareParameters()
  {
    ...
    exposure_time_ = this->declare_parameter("exposure_time", 3000, param_desc);

    CameraSetExposureTime(h_camera_, exposure_time_);
    RCLCPP_INFO(this->get_logger(), "Exposure time = %d", exposure_time_);
