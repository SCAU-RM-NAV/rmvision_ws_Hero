...
void SerialDriverNode::searchFrameSOF(uint8_t* frame, uint16_t total_len)
{
  ...
  for (i = 0; i < total_len;)
  {
    if (*frame == HEADER_SOF)
    {
      ...
      gimbal_fdb.aiming_mode = vision_tx_data.data.mode_msg.aiming_mode; //选择瞄准模式
      //std::cout<<gimbal_fdb.aiming_mode<<std::endl;

      gimbal_fdb.aiming_mode = 0;
...
void SerialDriverNode::publishTrajectoryMarker(float pitch, float yaw)
{
  ...
  for (int t = 0; t <= 20; t++)
  {
    ...
    const double s = (std::log(t0 * k * current_v * std::cos(-pitch) + 1.0) / k)+s_bias;
